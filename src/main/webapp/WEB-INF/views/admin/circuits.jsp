...
<head>
    ...
    <title>Gestion des Circuits - VoyageConnect Admin</title>
...
.nav-link:hover {
    ...
    background: var(--vc-border-light);
        ...
        .spots-ok { background: #dcfce7; color: #15803d; }
        .spots-low { background: #fef3c7; color: #a16207; }
        .spots-full { background: #fee2e2; color: #dc2626; }
...
<body>
    ...
    <div class="page-header">
        <div class="container">
            <div class="page-header-content">
                <div>
                    <h1><i class="fas fa-route"></i> Gestion des Circuits</h1>
                    <p class="subtitle">Créez et gérez les circuits touristiques de la plateforme</p>
                </div>
    ...
    <div class="container">
        ...
        <div class="stats-row">
            <div class="stat-mini">
                <div class="stat-mini-value">${circuits != null ? circuits.size() : 0}</div>
                <div class="stat-mini-label">Total Circuits</div>
            </div>
            <div class="stat-mini">
        ...
        <div class="admin-card">
            <div class="card-header">
                <h2><i class="fas fa-plus-circle"></i> Ajouter un nouveau circuit</h2>
            </div>
            <div class="card-body">
                <form action="${pageContext.request.contextPath}/admin/circuits" method="post">
                    <input type="hidden" name="action" value="create">
                    
                    <div class="form-grid">
                        <div class="form-group">
                            <label class="form-label">
                                <i class="fas fa-route"></i> Nom du circuit <span class="required">*</span>
                            </label>
                            <input type="text" name="name" class="form-control" placeholder="Ex: Tour des Kasbahs" required>
                        </div>
                        ...
                        <div class="form-group">
                            <label class="form-label">
                                <i class="fas fa-clock"></i> Durée (jours) <span class="required">*</span>
                            </label>
                            <input type="number" name="durationDays" class="form-control" placeholder="7" min="1" required>
                        </div>

                        <div class="form-group">
                            <label class="form-label">
                                <i class="fas fa-money-bill"></i> Prix (MAD) <span class="required">*</span>
                            </label>
                            <input type="number" name="price" class="form-control" placeholder="5000" min="0" step="0.01" required>
                        </div>

                        <div class="form-group">
                            <label class="form-label">
                                <i class="fas fa-calendar"></i> Date de début <span class="required">*</span>
                            </label>
                        ...
                        <div class="form-group">
                            <label class="form-label">
                                <i class="fas fa-users"></i> Places totales <span class="required">*</span>
                            </label>
                            <input type="number" name="totalSpots" class="form-control" placeholder="20" min="1" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="url" name="imageUrl" class="form-control" placeholder="https://example.com/circuit.jpg">
                        </div>
                        ...
                        <div class="form-group full-width">
                            ...
                            <textarea name="description" class="form-control" placeholder="Décrivez le circuit, les étapes, les points d'intérêt..."></textarea>
                        </div>

                        <div class="form-group">
                            <label class="form-label">
                                <i class="fas fa-check-circle"></i> Inclus dans le prix
                            </label>
                            <textarea name="included" class="form-control" placeholder="Transport, hébergement, repas, guide..."></textarea>
                        </div>

                        <div class="form-group">
                            <label class="form-label">
                                <i class="fas fa-times-circle"></i> Non inclus
                            </label>
                            <textarea name="excluded" class="form-control" placeholder="Billets d'avion, dépenses personnelles..."></textarea>
                        </div>
                    </div>

                    <div class="form-actions">
                        <button type="submit" class="btn btn-success">
                            <i class="fas fa-plus"></i> Ajouter le circuit
                        </button>
        ...
        <div class="admin-card">
            <div class="card-header">
                <h2><i class="fas fa-list"></i> Liste des circuits</h2>
            </div>
            <div class="card-body">
                <c:choose>
                    <c:when test="${not empty circuits}">
                            ...
                            <table class="admin-table">
                                ...
                                <tbody>
                                    <c:forEach items="${circuits}" var="circuit">
                                        <tr>
                                            <td><span class="circuit-name">${circuit.name}</span></td>
                                            <td>
                                                <span class="location-badge">
                                                    <i class="fas fa-map-marker-alt"></i> ${circuit.destination.name}
                                                </span>
                                            </td>
                                            <td>
                                                <span class="duration-badge">
                                                    <i class="fas fa-clock"></i> ${circuit.durationDays} jours
                                                </span>
                                            </td>
                                            <td>
                                                <div class="dates-info">
                                                    <div><i class="fas fa-play"></i> ${circuit.startDate}</div>
                                                    <div><i class="fas fa-stop"></i> ${circuit.endDate}</div>
                                                </div>
                                            </td>
                                            <td>
                                                <span class="price-tag">
                                                    <fmt:formatNumber value="${circuit.price}" type="number" maxFractionDigits="0"/> MAD
                                                </span>
                                            </td>
                                            <td>
                                                <c:choose>
                                                    <c:when test="${circuit.availableSpots > 5}">
                                                        <span class="spots-badge spots-ok">${circuit.availableSpots}/${circuit.totalSpots}</span>
                                                    </c:when>
                                                    <c:when test="${circuit.availableSpots > 0}">
                                                        <span class="spots-badge spots-low">${circuit.availableSpots}/${circuit.totalSpots}</span>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <span class="spots-badge spots-full">Complet</span>
                                                    </c:otherwise>
                                                </c:choose>
                                            </td>
                                            <td>
                                                <div class="action-buttons">
                                                    <button class="btn btn-warning btn-sm" onclick="editCircuit('${circuit.id}')">
                                                        <i class="fas fa-edit"></i>
                                                    </button>
                                                    <form action="${pageContext.request.contextPath}/admin/circuits" method="post" style="display: inline;" onsubmit="return confirm('Supprimer ce circuit ?');">
